...
class EventDetailViewController: UIViewController {
    ...
    func update() {
        ...
        if let imageName = event?.attachment {
        ...
        self.createTimeLabel.text = (event?.createTime?.dateTimeString() ?? Date().dateTimeString()) ?? ""
...
extension EventDetailViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell!.detailTextLabel?.text = "by \(entity.ownerName ?? ""), \(entity.createTime ?? Date())"
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let deleteAction = UIContextualAction(style: UIContextualAction.Style.normal, title: "Delete", handler: {(contextualAction, view, boolValue) in
            ...
            appMgr.delete(comment: document.documentID) {  [weak self] _,_ in
