...
class AddEventViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func submitEvent(_ sender: UIButton) {
        ...
        guard let groupId = groupModel?.uuid else {
            return
        }
        ...
        appMgr.addCrime(title: title, content: content,
                                      image: imgData, groupId: groupId) { success,error in
            ...
            if let err = error {
                ...
                if let gm = self.groupModel {
                    ...
                    appDB.updateGroup(groupId: groupId, eventCount: gm.eventCount) { (error) in
